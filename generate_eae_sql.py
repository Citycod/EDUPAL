...
text = ""
with open('eae_raw.txt', 'r', encoding='utf-8') as f:
    text += f.read() + "\n\n"

# Clean up random page numbers if any (e.g., "Education 45 New")
text = re.sub(r'Education \d+ New', '', text)
text = re.sub(r':\s*\n+', ':\n', text) # clean up floating colons

# Split blocks by course header starting with EAE
course_blocks = re.split(r'\n(?=EAE \d{3}:)', text.strip())

sql = "-- Seed file for NUC Adult Education Curriculum\nBEGIN;\n\n"
programs_seen = set()

for block in course_blocks:
    if not block.strip(): continue
    
    # Parse header: EAE 101: Introduction to Adult Education (2 Units C: LH 30)
    ...
    if not objectives:
        # Fallback to general sentence matching
...
with open('eae_seed.sql', 'w', encoding='utf-8') as f:
    f.write(sql)
    
print("Successfully generated eae_seed.sql")
