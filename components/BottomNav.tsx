...
export default function BottomNav({ navItems: customNavItems }: BottomNavProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 px-1 sm:px-4 pb-6 pt-2">
      <div className="mx-auto max-w-lg bg-white/80 dark:bg-[#0a120d]/80 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-[2.5rem] shadow-2xl shadow-black/20 px-1 sm:px-2 py-2 flex justify-between items-center transition-all duration-300">
        {navItems.map((item: any, index: number) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-0.5 sm:gap-1 flex-1 px-0 group outline-none"
            >
              <div
                className={`relative px-2 sm:px-5 py-2 rounded-2xl transition-all duration-300 ${itemActive
              ...
              >
                <span className={`material-symbols-outlined text-[20px] sm:text-[24px] block transition-transform duration-300 ${item.filled && itemActive ? 'fill-1' : ''
              ...
              <span
                className={`text-[8px] sm:text-[10px] font-black uppercase tracking-tight sm:tracking-widest transition-colors duration-300 ${itemActive ? 'text-primary' : 'text-slate-500 dark:text-slate-500 opacity-0 group-hover:opacity-100'
                  }`}
              >
                {item.label}
              </span>
